...
import {
  listManagerManualRequests,
  decideManagerManualRequest,
...
export default function ManagerManualRequests() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      const data = await listManagerManualRequests(token, tab);
      setItems(data || []);
    } catch (err: any) {
      Alert.alert(
  ...
  const onDecide = async (action: "APPROVE" | "REJECT") => {
    ...
    try {
      const token = await AsyncStorage.getItem("token");
      if (!token) return;
      await decideManagerManualRequest(token, selected.id, {
        action,
